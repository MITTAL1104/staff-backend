...
public class AllocationExcelExportServiceImpl implements AllocationExcelExportService {
    ...
    @Async("taskExecutor")
    @Override
    public void generateExcelAsync(List<Allocation> allocations, HttpServletResponse response) {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            for (Allocation alloc : allocations) {
                ...
                row.createCell(7).setCellValue(alloc.getIsActive() == TRUE ? "Active" : "Inactive");
